...
1. 4人対局エンジンに `pon` / `chi` / `kan` イベントと副露状態を追加
2. `world` が白發中・自風・場風を役牌ポンできるようにした
3. `world` が喰いタンのポン/チーをできるようにした
4. `world` が役牌大明槓をできる最小実装を追加
5. 副露後の簡易打牌、開いた手の最低限の和了判定、役牌/喰いタン副露の簡易役判定を追加

確認済み:

- `npm test`
- `npm run build`
- `npm run tournament -- 20 42 random,simple-shanten,attacker,world`
